...
document.addEventListener( 'DOMContentLoaded', function () {
	...
	const videoTestimonialBlocks = document.querySelectorAll(
	...
	function initBlock( block ) {
		...
		const thumbnailsContainer = block.querySelector(
		...
		if ( playerContainer && videos.length > 0 && videos[ selectedVideo ] ) {
			...
			playerContainer.innerHTML = `
                <div class="video-placeholder">
                    ...
                    <p>${ __( 'No video selected', 'video-testimonial' ) }</p>
                </div>
            `;
		}
	}
	...
	function renderThumbnails( state ) {
		const { videos, thumbnailsContainer } = state;
	...
	function renderCombobox( state ) {
		const { videos, thumbnailsContainer, playerContainer } = state;
		...
		input.addEventListener( 'keydown', function ( e ) {
			...
			if ( e.key === 'ArrowDown' ) {
				...
				e.preventDefault();
				if ( visible[ pos ] )
					selectOption( optionItems.indexOf( visible[ pos ] ) );
	...
	function renderDesktopThumbnails( state ) {
		const { videos, videoLayout, thumbnailsContainer, playerContainer } =
			state;
		...
		videos.forEach( ( video, index ) => {
			...
			thumbnailButton.className = `video-thumbnail ${
				state.currentIndex === index ? 'selected' : ''
			}`;
		...
		if ( video.type === 'youtube' ) {
			...
			container.innerHTML = `
                ...
                <div class="video-player-container video-player-youtube">
                    <div class="video-youtube-poster" style="${
						displayImage
							? `background-image:url(${ displayImage });`
							: ''
					}background-size:cover;background-position:center;width:100%;aspect-ratio:16/9;">
			...
			if ( playButton && posterEl && playerContainer ) {
				playButton.addEventListener( 'click', function () {
					const autoplayUrl = video.url.includes( '?' )
